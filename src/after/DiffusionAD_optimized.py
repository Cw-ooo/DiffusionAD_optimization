...
class PrecomputedDefectDataset(Dataset):
    ...
    def _build_normal_index(self) -> dict[str, Path]:
        ...
        for normal_path in sorted(self.normal_dir.iterdir()):
            if not self._is_image_file(normal_path):
                continue
    ...
    def _build_pairs(self) -> list[tuple[Path, Path]]:
        ...
        for defect_path in sorted(self.defect_dir.iterdir()):
            if not self._is_image_file(defect_path):
                continue
            ...
            try:
                ...
                raise FileNotFoundError(
                    f"'{defect_path.name}'에 대응되는 정상 이미지가 없습니다: {normal_filename}"
                ) from e
            pairs.append((normal_path, defect_path))
            
        if not pairs:
            raise ValueError(f"유효한 학습 pair가 없습니다: {self.defect_dir}")
        return pairs
...
class InferenceVisualizer:
    ...
    def save_result(self, D_orig, L_final, S_final, save_path, title_prefix=""):
        ...
        axes[0].set_title(f'{title_prefix}Defect Image')
        axes[0].axis('off')
...
def run_inference_on_folder(config, model, vae, noise_scheduler, device):
    ...
    for img_path in tqdm(list(image_iter), desc="Folder inference"): # tqdm requires length, or drop tqdm
...
class DefectTrainer:
    ...
    @profiled("Trainer._build_components")
...
def main(run_inference=False):
    ...
    config = TrainConfig(**base_kwargs)
    print("=" * 60)
    print("최적화된 코드로 학습 시작")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    main(run_inference=args.inference)
